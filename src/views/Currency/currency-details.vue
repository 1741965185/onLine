<template>
	<div class="currency-container">
		...
		<div class="handle-title">
				...
				<div class="element">
					<div class="custom-div" @click="$router.go(-1)">
						<img  src="@/assets/images/SketchPngdbe36b5b576ccbe92f372c4a7a98eedf33de502e8fc4dc859b05f3486f3da89d.png"  alt="" />
						<span class="back-text">{{ $t('login.back') }}</span>
					</div>
				</div>
				<div style="padding-bottom: 20px"></div>
			<img style="width: 63px;   border-radius: 50%;
height: 64px;" :src="`/currency/${currentCurrency}.png`" alt="" />
			<div style="padding-bottom: 8px"></div>
			<div style="font-family: Roboto, Roboto;
font-weight: 500;
font-size: 24px;
color: #364A63;
line-height: 26px;
...
text-align: left;
font-style: normal;
">{{ $t('sysText.home_total_balance') }}</div>
			<div style="padding-bottom: 10px"></div>
			<!-- 修复CSS样式中重复定义字体的问题 -->

			<div style="font-family: Roboto;">
				<span class="custom-font">$</span>
				<span class="custom-font">	{{this.currentCurrencyBalance}}</span>
...
"></div>
			...
			<!--只提现//充值 -->
			<div style="display: flex" v-if="!isDigit">

				<div  style="display: flex;flex-direction: column;align-items: center;justify-content: center;margin-right: 40px">
					<div class="chongzhi-button" @click="showDialog('recharge')">
						<svg-icon class="svg-tabs" icon-class="chongzhi-main"/>
					</div>
					<div class="btn-text-te">{{$t('sysText.home_add_money')}}</div>
				</div>
				<div style="display: flex;flex-direction: column;align-items: center;justify-content: center;margin-right: 40px">
					<div class="chongzhi-button2" @click="showDialog('withdraw')" v-if="merType === 0">
						<svg-icon class="svg-tabs" icon-class="hunkuan-mian"/>
					</div>
					<span class="btn-text-te2">{{$t('sysText.home_send')}}</span>
				</div>
				<div style="display: flex;flex-direction: column;align-items: center;justify-content: center">
					<div class="chongzhi-button2" @click="showDialog('share')">
						<svg-icon class="svg-tabs" icon-class="fengxiang-mian"/>
					</div>
					<span class="btn-text-te2">{{$t('sysText.home_create')}}</span>
				</div>
			</div>
			...
			<div style="display: flex" v-if="isDigit" >
				...
				<div  style="display: flex;flex-direction: column;align-items: center;justify-content: center;margin-right: 40px">
					...
					<div class="btn-text-te">{{$t('sysText.home_buy_and_sell_with_cash')}}</div>
				</div>
				<div style="display: flex;flex-direction: column;align-items: center;justify-content: center;margin-right: 40px">
					<div class="chongzhi-button2" @click="showDialog('newRechange')" v-if="merType === 0">
						<svg-icon class="svg-tabs" icon-class="chongzhi"/>
					</div>
					<span class="btn-text-te2">{{$t('sysText.home_deposit')}}</span>
				</div>
				<div style="display: flex;flex-direction: column;align-items: center;justify-content: center">
					<div class="chongzhi-button2" @click="showDialog('digitTransfer')">
						<svg-icon class="svg-tabs" icon-class="hunkuan-mian"/>
					</div>
...
">																{{$t('sysText.qr_code')}}
		...
		<!--提现一次弹窗-->
		<el-dialog :visible.sync="withdrawalVisible"  width="500px"  @close="closeDialog"  center>
			<div style="margin-top: -10%;text-align:center;margin-left: 40px;	margin-bottom: 10px;cursor: pointer">
				<el-dropdown @command="selectTXMerchant" placement="bottom">
					<div style="display: flex;	flex-direction: row;	justify-content: space-between;align-content: center">
						<span class="text_5">{{$t('sysText.withdrawalMerchant')+selectedMerchant}}</span>
						<img
							class="thumbnail_2"
							referrerpolicy="no-referrer"
							src="https://lanhu.oss-cn-beijing.aliyuncs.com/SketchPng1f17c18905e26eb8335e786243996dfff041421d4eab6ab6a46f8855540290ab"
						/>
					</div>
					<el-dropdown-menu class="custom-dropdown" style="width: 169px;margin-top: -2px">
						<el-dropdown-item v-for="(merchant) in merchantList" :key="merchant"  :command="merchant">
							{{ merchant }}
						</el-dropdown-item>
					</el-dropdown-menu>
				</el-dropdown>
...
font-weight: 500;
color: #211F32;
line-height: 20px; ">{{$t('home.digitalCurrency')}}</span>
				<div style="margin-top: 10px" @click="twoWithdraw('USDT')">
					<el-button style="width: 100%">
						<span style="float: left">USDT</span>
						<span style="float: right">{{overAmount.USDT_TRC==null?0:overAmount.USDT_TRC}} </span>
					</el-button>
				</div>
...
font-weight: 500;
color: #211F32;
line-height: 20px; ">{{$t('home.fiat')}}</div>
			</div>
			<div style="margin-top: 10px" @click="twoWithdraw('USD')">
				<el-button style="width: 100%">
					<span style="float: left">USD</span>
					<span style="float: right">{{overAmount.USD==null?0:overAmount.USD}}  </span>
				</el-button>
			</div>
			<div style="margin-top: 10px" @click="twoWithdraw('HKD')" >
				<el-button style="width: 100%">
					<span style="float: left">HKD</span>
					<span style="float: right">{{overAmount.HKD==null?0:overAmount.HKD}}  </span>
				</el-button>
			</div>
			<div style="margin-top: 10px" @click="twoWithdraw('BRL')" >
				<el-button style="width: 100%">
					<span style="float: left">BRL</span>
					<span style="float: right">{{overAmount.BRL==null?0:overAmount.BRL}}  </span>
				</el-button>
			</div>
			<!--			<div style="width: 10px;margin-top: 20px"></div>-->
			<div style="margin-top: 10px" @click="twoWithdraw('PHP')" >
				<el-button style="width: 100%">
					<span style="float: left">PHP</span>
					<span style="float: right">{{overAmount.PHP==null?0:overAmount.PHP}}  </span>
				</el-button>
			</div>
			<div style="width: 10px;margin-top: 20px"></div>
		</el-dialog>
		...
		<!--提现二次弹窗-->
		<el-dialog :visible.sync="withdra1walVisible" :title="$t('home.withdrawal')" width="500px"  center>
			<div :style="{height: getHeight()}">
				<el-row>
					<el-col :span="11">

						<el-select v-model="citiesValue" :placeholder="$t('home.select')" style="width: 180px;margin-top: 10px"  @change="selectValue()" >
							<el-option
					...
					<el-col :span="3">
						<div style="width: 24px;height: 24px; margin-top: 20px"> <img src="@/assets/images/jt.png"> </div>
					</el-col>
					<el-col :span="8">
						<el-select v-model="cities1Value" :placeholder="$t('home.select')" style="width: 180px;margin-top: 10px" @change="selectValue()">
							<el-option
	...
	font-weight: 500;
	color: #7F8DFF;
	line-height: 20px;">1{{citiesValue}}={{1 * rateAndBankInfo.rate}}{{cities1Value}}</span>
				<div v-if="withdraType==1">
					<div style="margin-top: 24px;font-size: 14px;float: left;">{{$t('home.withdrawalAmount')}}</div>
					<div style="margin-top: 24px;font-size: 14px;float:right;color: #8094AE;">{{$t('home.availableBalance')}}{{overAmount[citiesValue == 'USDT'?citiesValue+'_TRC':citiesValue]}} {{citiesValue}}</div>
					<div style="margin-top: 8px">
						<el-input placeholder="0.00" v-model="withdrawAmount">
							<template slot="append"><span style="color: #364A63">{{citiesValue}}</span></template>
						</el-input>
					</div>
					<span style="font-size: 12px;">{{$t('home.fee')}}{{(withdrawAmount * (rateAndBankInfo.fee * 0.01)).toFixed(4) }} {{citiesValue}}</span>
					<div style="margin-top: 24px">{{$t('home.beneficiaryAccount')}}</div>

					<el-select v-model="selectedOption" :placeholder="$t('home.select')" style="margin-top: 10px" @change="selectValue()">
						<el-option :label="$t('home.withdrawingYourself')" value="0"></el-option>
						<el-option :label="$t('home.transferCompanies')" value="1"></el-option>
					</el-select>


					<div v-if="this.selectedOption==='0'">
						<div style="display: flex; flex-direction: column;">
							<div style="display: flex; flex-direction: row;justify-content: space-between;align-items: center;margin-top: 18px;">
								<div>{{$t('home.beneficiaryBank')}}</div>
								<div class="text_sdgsga">{{rateAndBankInfo.bank}}</div>
							</div>
							<div style="display: flex; flex-direction: row;justify-content: space-between;align-items: center;	margin-top: 18px;">
								<div>{{$t('home.beneficiaryCardNumber')}}</div>
								<div class="text_sdgsga">{{rateAndBankInfo.account}}</div>
							</div>
							<div style="display: flex; flex-direction: row;justify-content: space-between;align-items: center;	margin-top: 18px;">
								<div>{{$t('home.beneficiaryName')}}</div>
								<div class="text_sdgsga">{{rateAndBankInfo.name}}</div>
							</div>
						</div>
					...
					<div v-else style="margin-top: 12px">
						<template >
							<el-select v-model="country" :placeholder="$t('home.Country')">
								<el-option
						...
						<el-input :placeholder="$t('home.beneficiaryCardNumber')" style="margin-top: 12px" v-model="payee">
						</el-input><el-input :placeholder="$t('home.beneficiaryName')" style="margin-top: 12px" v-model="payeeName">
					</el-input><el-input placeholder="Swift" style="margin-top: 12px" v-model="swift">
...
font-weight: 400;
color: #8094AE;
line-height: 18px;">{{$t('withdrawDetails.expectedReceive')}}</div>
					</div>

				</div>

				<div v-if="withdraType==2">
					<div style="margin-top: 24px">{{$t('home.internet')}}</div>
					<div style="margin-top: 8px">
						<el-button type="primary" style="width: 98px;
height: 34px;
font-size: 14px;">TRC20</el-button>
					</div>
					<div style="margin-top: 24px;font-size: 14px;float: left;">{{$t('home.quantity')}}</div>
					<div style="margin-top: 24px;font-size: 14px;float:right;color: #8094AE;">{{$t('home.availableBalance')}}{{(overAmount[citiesValue == 'USDT'?citiesValue+'_TRC':citiesValue])==null?'0':(overAmount[citiesValue == 'USDT'?citiesValue+'_TRC':citiesValue])}} {{citiesValue}}</div>
					<el-input style="margin-top: 7px;" v-model="withdrawAmount" placeholder="0.00" >
						<template slot="append"><span style="color: #364A63">{{citiesValue}}</span></template>
					</el-input>
					<span style="font-size: 12px;">{{$t('home.fee')}}{{(withdrawAmount * (rateAndBankInfo.fee * 0.01)).toFixed(4)}} {{citiesValue}}</span>
					<div style="margin-top: 25px">{{$t('home.withdrawTo')}}</div>
					<el-input  :placeholder="$t('home.walletAddress')" v-model="numberAddress" ></el-input>
				</div>


				<div style="float: right;margin-top: 24px">
					<el-button type="primary" :disabled="disabled" @click="addWithdraw()">{{$t('home.withdraw')}}</el-button>
				</div>
				<div style="float: right;margin-top: 24px;margin-right: 24px;">
					<div>
						<span style="font-size: 12px;color: #52575C;">{{$t('home.actualReceived')}}</span><br/>
						<span style="font-size: 16px;color: #364A63;">{{(withdrawAmount * rateAndBankInfo.rate).toFixed(4)}} {{cities1Value}}</span>
					</div>
				</div>
			</div>
		</el-dialog>
		...
		<div v-if="isOpenCurrencyModalVisible" class="dialog-overlay">
			<div class="modal-content-openCurrency">
				<div style="display: flex;justify-content: space-between;align-items: center;width: 100%">
					...
					<img class="close-button" src="@/assets/images/close.png" @click="hideOpenCurrencyModal" alt="关闭按钮" />
				</div>
...
import {
	...
	checkStandPay,
	merQueryAccount,
	addAccountBalance, getCurrencyDetail
} from "@/api/home";
import {
...
export default {
	...
	mounted() {
		...
		if(this.currentCurrency === 'USDT' || this.currentCurrency === 'ETH' || this.currentCurrency === 'BTC' || this.currentCurrency ==='USDC'){
			this.isDigit = true;
		}
	...
	async	gotoCertification()	{
		...
		confirmOpenCurrency() {
				...
				// 处理确定按钮点击事件
				// 可以在这里执行提现操作等逻辑
				this.router.push({ path: "/openCurrency" });
		...
		merHomeQueryAccount(){
			getCurrencyDetail( {
				...
				if (res.status === 200) {
					this.currentCurrencyBalance=res.data;
				}
			});
		},
		...
		addWithdraw(){
			...
			if(this.withdraType ==1){
				if (this.selectedOption==="0"){
					body.bank = this.rateAndBankInfo.bank;
					body.bankNumber = this.rateAndBankInfo.account;
					body.name = this.rateAndBankInfo.name;
					body.sourceCurrency = this.citiesValue;
					body.targetCurrency = this.cities1Value;
					body.type=this.selectedOption
					body.destinationType = this.rateAndBankInfo.type;
				}else{
					body.type=this.selectedOption
				...
				body.bank = 'TRC20';
				body.bankNumber = this.numberAddress;
				body.name = '';
				body.sourceCurrency = this.citiesValue;
				body.targetCurrency = this.cities1Value;
				body.destinationType = this.rateAndBankInfo.type;
		...
		isProGen() {
				...
				this.shareLink='';
				this.seOneCurrency=this.$route.query.currency;
				this.amount='';
				this.remark='';
				this.isPro='0';
			this.$nextTick(() => {
		...
		async showDialog(type) {
			...
			if (type === 'share') {
				...
				this.$router.push({ path: '/recharge', query: { currency: this.currentCurrency }})
			} else if (type === 'withdraw') {
				// this.dialogTitle =  this.$t('home.selectWithdrawalCurrency');
				this.$router.push({ path: '/moneyTransfer' , query: { currency: this.currentCurrency }})

				this.withdrawalVisible = true;
				getAmount({merNo:this.selectedMerchant}).then(res=>{
					this.overAmount=res.data;
				})
			}else if (type === 'digitTransfer') {
				// this.dialogTitle =  this.$t('home.selectWithdrawalCurrency');
				this.$router.push({ path: '/digitTransfer' , query: { currency: this.currentCurrency }})
